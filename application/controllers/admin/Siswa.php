...
class Siswa extends CI_Controller { 
	...
	public function index($status = "Daftar")
	{
		...
		if($status == "verified")
		{
			...
			$students = $this->Student->get(['status="Daftar Ulang" OR status="Lulus" OR status='=>'Terverifikasi']);
...
// 			'verification_by' => $this->session->userdata('id')
		...
		// $this->whatsapp->send($student->phone,'after_verified',$student);
		$this->session->set_flashdata('success', "Calon siswa berhasil diluluskan.");
		redirect('admin/siswa');
		return;
	}
	...
	function print($id)
	{
		...
		$siswa = $this->Student->find(['id'=>$id,'status'=>'Daftar Ulang']);
		if(!$siswa)
		{
			redirect('admin');
			return;
		...
		$ringkasan = $this->load->view('admin/siswa/tanda-terima',[
			'siswa' => $siswa,
			'pas_foto' => $pas_foto,
			'qrcode' => $base64,
			'kop' => $kop,
