...
class Daftar extends CI_Controller {
	...
	function index($step = 1)
	{
		...
		elseif($step > 2)
		{
			...
			if($step == 3)
			{
				...
				$data_jenjang = $daftar['data_jenjang'];
				$check_student = $this->Student->check($daftar['data_pribadi']['NIK'], $data_jenjang);
				if($check_student)
				{
		...
		if($step == 6)
		{
			...
			foreach($_FILES as $key => $value)
			{
				...
				$size = filesize($_FILES[$key]['tmp_name']) / 1024;

				if($size > 500)
				{
					$this->session->set_flashdata('upload_error', 'File tidak boleh lebih dari 500Kb');
					// After that you need to used redirect function instead of load view such as 
					redirect(base_url('daftar/index/5'));
					break;
				}
			...
			else
			{
				...
				// $this->whatsapp->send($student->phone,'after_register',$student);
				// $this->db->insert('queue_jobs',[
                //     'name' => 'after_register',
                //     'data' => serialize((array) $student),
                // ]);
		...
		$labels = [
			...
			'kesehatan' => [
				...
				'imudity' => 'Imunisasi',
				'eye_problem_description' => 'Masalah Mata',
	...
	function buktiPendaftaran($id)
	{
		...
		$path = 'public/qrcode/'.$siswa->register_number.'.png';
		$qrcode = base_url($path);
		return $this->load->view('daftar/bukti-pendaftaran',[
			'siswa' => $siswa,
			'pas_foto' => $pas_foto[0],
			'qrcode' => $qrcode,
		]);
